...
class TrajOpt:
    ...
    def TrajGeneratorFromPFreeRot(self, preds, step): 
        # Points is in se3
        # step has a default value at 0.1
        batch_size, num_p, dims = preds.shape
        points_preds = torch.cat((torch.zeros(batch_size, 1, dims, device=preds.device, requires_grad=preds.requires_grad), preds), axis=1) # Concat with the zeros (current pos of robot)
        num_p = num_p + 1
        xs = torch.arange(0, num_p-1+step, step, device=preds.device) # Interpolate every step
